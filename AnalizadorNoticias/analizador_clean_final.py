...
def extraer_titulares(texto):
    """Extrae titulares de manera más flexible."""
    lineas = texto.split('\n')
    titulares = []
    for linea in lineas:
        linea = linea.strip()
        if not linea:
            continue
        # Capturar líneas que comienzan con mayúscula o número
        if (linea[0].isupper() or linea[0].isdigit()) and len(linea) > 10:
            # Excluir solo metadatos claros
            if not any(meta in linea for meta in ["--- Página", "📰", "✍️", "📂", "SÍNTESIS", "COORDINACIÓN"]):
                titulares.append(linea)
    return titulares[:40]  # Permitir más titulares
...
class AnalizadorGUI:
    ...
    def mostrar_resultados(self, importantes, sin_eco, contexto):
        ...
        # Noticias sin eco
                # Siempre mostrar noticias sin eco
        self.resultados_texto.insert(tk.END, "═" * 70 + "\n\n", "separador")
        ...
        if sin_eco:
            ...
            self.resultados_texto.insert(tk.END, "   ✅ No se detectaron noticias sin eco.\n", "texto")
        ...
        self.resultados_texto.tag_config("causa", foreground="#f39c12", font=("Arial", 9))
